    ...
    if ($location instanceof Station2) {
        ...
        foreach ($location->getLines() as $line) {
            foreach ($line->getConnections() as $connection) {
                ...
                $cid = $connection->id;
                outputdebug("Looking at next connection " . $cid, $debug);
                ...
                if ($nextStation->isJunction()) {
                    ...
                    if ($originalBox->getArea() > $path->getBox()->getArea()) {
...
function expandPath ($path) {
    ...
    foreach ($currentStation->getLines() as $line) {
        ...
        foreach ($line->getConnections() as $connection) {
            ...
            $path->addSegment(new Segment($currentStation, $nextStation, $currentStation->getOverlapLines($nextStation), $connection->duration, $connection->type));
...
function algorithm ($start, $end) {
    ...
    */
    ...
    //return algorithmLoop($startPaths, $endPaths);
